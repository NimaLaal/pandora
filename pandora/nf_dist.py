...
class ValidationHell(object):
    ...
    def sample_from_nf(self, 
        ...
        if self.is_conditional:
            ...
            for ii in trange(context_samples.shape[0], colour="green", desc = "Validation: Sampling from NF"):
                gen_samples = gen_samples.at[ii].set(jnp.array(nf.condition(context_samples[None, ii]).sample((ndraws, )).T))
...
class NFMaker(object):
    ...
    def __init__(
        ...
        input_base = dist.Normal(loc = torch.zeros(self.input_dim, device = device), 
        ...
        scaling_transform = T.AffineTransform(loc=torch.zeros(self.input_dim, device = device), 
                                              scale=torch.ones(self.input_dim, device = device) * self.B+1)  # scale from (-1,1) to (-B-1,B+1)

        ## Constructing the spline flow either as a conditional or as a marginal flow
        if self.context_exist:
            input_hypernet = ConditionalAutoRegressiveNN(self.input_dim, 
                                                    self.context_dim, 
                                                    hidden_dims, 
            ...
            input_transform = T.ConditionalSplineAutoregressive(self.input_dim, 
                                                            input_hypernet, 
            ...
            input_hypernet = AutoRegressiveNN(self.input_dim, 
                                            hidden_dims, 
                                            param_dims=param_dims)
            if self.device == torch.device("cuda"):
                input_hypernet = input_hypernet.cuda()

            input_transform = T.SplineAutoregressive(self.input_dim, 
                                                    input_hypernet, 
                                                    count_bins=spline_bins, 
                                                    order = 'quadratic',
                                                    bound = self.B+1)
        ...
        if not start_from_loaded_nf:
            if self.context_exist:
                self.flow = dist.ConditionalTransformedDistribution(input_base, [tanh_transform, scaling_transform, input_transform])
            else:
                self.flow = dist.TransformedDistribution(input_base, [tanh_transform, scaling_transform, input_transform])
        else:
