...
def jax_efficient_normal_logpdf(x, loc=0.0, scale=1.0):
    """Efficient log PDF of a normal distribution."""
    log_unnormalized = -0.5 * ((x - loc) / scale)**2
    log_normalization = jnp.log(scale) + 0.5 * jnp.log(2.0 * jnp.pi)
    return log_unnormalized - log_normalization

def numpy_efficient_normal_logpdf(x, loc=0.0, scale=1.0):
    """Efficient log PDF of a normal distribution."""
    log_unnormalized = -0.5 * ((x - loc) / scale)**2
    log_normalization = np.log(scale) + 0.5 * np.log(2.0 * np.pi)
    return log_unnormalized - log_normalization
...
class UniformPrior(object):
    ...
    def __init__(
        ...
        self.f_dm = None
        self.GWB_fidxs = jnp.array(
            range(first_crn_bin_index, self.crn_bins + first_crn_bin_index)
        )
        self.nonGWB_fidxs = jnp.array(
            [ii for ii in range(self.int_bins) if not ii in self.GWB_fidxs]
        )
        ...
        if self.nonGWB_fidxs.any():
            self.DIR = jnp.repeat(
                self.diag_idx[None, :], len(self.nonGWB_fidxs), axis=0
            )
        ...
        psd_func_sigs = np.array(
            [
                ...
                2:
            ]  # the first 2 are skipped because their are `f``, and `df``
        )
        orf_func_signs = np.array(
            [
                ...
                1:
            ]  # the first one is skipped because it is `angle`
        )
    ...
    def get_phi_mat(self, xs):
        ...
        if self.orf_fixed:
            ...
            return phi.at[self.KGW, self.I, self.J].set(self.orf_val * psd_common)
        else:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
    ...
    def get_phi_mat_and_common_psd(self, xs):
        ...
        if self.orf_fixed:
            ...
            # and the slices are 2D
            return (
                phi.at[self.KGW, self.I, self.J].set(self.orf_val * psd_common),
                psd_common,
            )
        else:
            # Need to use multi-dimensional indexing as the array is 3D
            # and the slices are 2D
            return (
                phi.at[self.KGW, self.I, self.J].set(
                    self.orf_func(self.xi, *xs[self.gwb_psd_params_end_idx :])
                    * psd_common
                ),
                psd_common,
            )
    ...
    def get_phi_mat_inv(self, phi):
        ...
        if self.separate_inversion_strat:
            ...
            return jnp.repeat(
                phiinv.at[self.KIR, self.DIR, self.DIR].set(1 / diags), 2, axis=0
...
class UniformPriorDM(object):
    ...
    def __init__(
        ...
        self.DM_fidxs = jnp.array(
            [ii for ii in range(self.first_dm_bin_index, self.last_dm_bin_index)]
        )
        self.GWB_fidxs = jnp.array(
            range(self.first_crn_bin_index, self.last_crn_bin_index)
        )
        self.nonGWB_fidxs = jnp.array(
            [
                ii
                for ii in range(self.first_irn_bin_index, self.last_irn_bin_index)
                if not ii in self.GWB_fidxs
            ]
        )
        ...
        self.upper_prior_lim_all = jnp.zeros(self.num_IR_params)
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[0::4].set(
            self.gamma_dm_max
        )
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[1::4].set(
            self.log10A_dm_max
        )
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[2::4].set(self.gamma_max)
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[3::4].set(
            self.log10A_max
        )

        self.lower_prior_lim_all = jnp.zeros(self.num_IR_params)
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[0::4].set(
            self.gamma_dm_min
        )
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[1::4].set(
            self.log10A_dm_min
        )
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[2::4].set(self.gamma_min)
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[3::4].set(
            self.log10A_min
        )
    ...
    @partial(jax.jit, static_argnums=(0,))
    def get_phi_diag(self, log10ampIRN, gammaIRN, log10ampDM, gammaDM, gwb_psd_params):
        """
        Calculates the diagonal of the phi-matrix based on input parameters `log10amp`, `gamma`, and
        `gwb_psd_params`.
    ...
    def get_phi_mat(self, xs):
        ...
        phi_dm, phi_irn, psd_common = self.get_phi_diag(
            log10ampIRN, gammaIRN, log10ampDM, gammaDM, gwb_psd_params
        )
    ...
    def get_phi_mat_inv(self, phi):
        ...
        if self.separate_inversion_strat:
            ...
            return jnp.repeat(phiinv, 2, axis=0), 2 * (
                logdet_phi + jnp.sum(jnp.log(diags)) + jnp.sum(jnp.log(diags_dm))
            )
        else:
            return jnp.repeat(phiinv, 2, axis=0), 2 * (
                logdet_phi + jnp.sum(jnp.log(diags_dm))
            )
...
class UniformPriorGwbOnly(object):
    ...
    def __init__(
        ...
        self.upper_prior_lim_all = jnp.array(
            gwb_helper_dictionary["gwb_psd_param_upper_lim"]
        )
        self.lower_prior_lim_all = jnp.array(
            gwb_helper_dictionary["gwb_psd_param_lower_lim"]
        )
        ...
        self.gwb_psd_params_end_idx = len(self.gwb_psd_varied_param_indxs)
    ...
    def get_phi_mat(self, xs):
        ...
        phi = phi.at[:, self.diag_idx, self.diag_idx].set(phi_diag)
        if self.orf_fixed:
            return phi.at[:, self.I, self.J].set(self.orf_val * psd_common)
        else:
    ...
    def get_phi_mat_and_common_psd(self, xs):
        ...
        if self.orf_fixed:
            return (
                phi.at[: self.crn_bins, self.I, self.J].set(self.orf_val * psd_common),
                psd_common,
            )
        else:
            return (
                phi.at[:, self.I, self.J].set(
                    self.orf_func(self.xi, *xs[self.gwb_psd_params_end_idx :])
                    * psd_common
                ),
                psd_common,
...
class TransverseVL(object):
    ...
    def __init__(
        ...
        self.diag_idx = jnp.arange(0, self.Npulsars)
        self.ppair_number = int(self.Npulsars * (self.Npulsars - 1) * 0.5)
        self.num_IR_params = 3 * Npulsars
        self.renorm_const = renorm_const

        #Pulsar distance mean and std
        self.pdist_mean = jnp.array(pdist_mean_std[:, 0])
        self.pdist_std = jnp.array(pdist_mean_std[:, 1])
        ...
        self.upper_prior_lim_all = jnp.zeros(self.num_IR_params)
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[0::3].set(self.gamma_max)
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[1::3].set(self.log10A_max)
        self.upper_prior_lim_all = self.upper_prior_lim_all.at[2::3].set(10.0)

        self.lower_prior_lim_all = jnp.zeros(self.num_IR_params)
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[0::3].set(self.gamma_min)
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[1::3].set(self.log10A_min)
        self.lower_prior_lim_all = self.lower_prior_lim_all.at[2::3].set(0.0)
    ...
    @partial(jax.jit, static_argnums=(0,))
    def get_phi_diag(self, log10amp, gamma, pulsar_dists, gwb_psd_params):
        """
        Calculates the diagonal of the phi-matrix based on input parameters `log10amp`, `gamma`, and
        `gwb_psd_params`.
        ...
        2. the common process PSD
        """
        psd_common_total, comp_psds = self.gwb_psd_func(
            self.f_common,
            self.df,
            pulsar_dists,
            *self.param_value_container.at[self.gwb_psd_varied_param_indxs].set(
        ...
        # No need to use multi-dimensional indexing as the array is 2D
        # and the slices are 1D
        return phi_diag.at[self.GWB_fidxs].add(psd_common_total), comp_psds
    ...
    def get_phi_mat(self, xs):
        ...
        :return: the phi-matrix` with dimensions `(n_f,n_p, n_p)`.
        """
        log10amp, gamma, pulsar_dists, gwb_psd_params = (
            xs[1 : self.num_IR_params : 3],
            xs[0 : self.num_IR_params : 3],
            xs[2 : self.num_IR_params : 3],
            xs[self.num_IR_params : self.gwb_psd_params_end_idx],
        )
        phi_diag, comp_psds = self.get_phi_diag(log10amp, gamma, pulsar_dists, gwb_psd_params)
        phi = jnp.zeros((self.int_bins, self.Npulsars, self.Npulsars))
        phi = phi.at[:, self.diag_idx, self.diag_idx].set(phi_diag)

        # Need to use multi-dimensional indexing as the array is 3D
        # and the slices are 2D
        # Also, note that: [n_orfs, 1, n_pairs] * [n_orfs, n_f, 1] = [n_orfs, n_f, n_pairs] 
        return phi.at[self.KGW, self.I, self.J].set((self.orf_val[:, None, :] * comp_psds).sum(axis = 0))
    ...
    def get_phi_mat_CURN(self, xs):
        ...
        return self.get_phi_diag(log10amp, gamma, pulsar_dists, gwb_psd_params)
    ...
    def get_lnprior(self, xs):
        ...
        pdist_prior = jax_efficient_normal_logpdf(xs[2 : self.num_IR_params : 3], 
                                            loc=self.pdist_mean,
                                            scale=self.pdist_std).sum()
        return pdist_prior + jax.lax.cond(state, self.spit_neg_number, self.spit_neg_infinity)
...
class VL(object):
    """
    A class to take care of prior and the phi-matrix construction for a VL model based
    on log-uniform priors.
    NOTE: this class is for achromatic IRN + GWB
    ...
    def __init__(
        ...
        #Mimicking what is done above for TTVL below. 
        #Made sure things were still called correctly
        ...
        #left in just in case it gets used but will probably only use powerlaws
        if "halflog10_rho" in psd_func_sigs:
            # The function signature of the free-spectral model
            # is different from others. Hence, it needs its own logic.
            self.param_value_container = jnp.zeros(self.crn_bins)
        ...
        # This is the index where the ORF parameters begin and PSD params end
        self.gwb_psd_params_end_idx = self.num_IR_params + len(
            self.gwb_psd_varied_param_indxs
        )
    ...
    def get_phi_diag(self, log10amp, gamma, pulsar_dists, gwb_psd_params):
        ...
        2. the common process PSD
        """
        psd_common_total, vl_psd = self.gwb_psd_func(
            self.f_common,
            self.df,
            pulsar_dists,
            *self.param_value_container.at[self.gwb_psd_varied_param_indxs].set(
        ...
        #Not sure how this is working quite yet. There is a diagonal power and we add on top of it? 
        phi_diag = self.powerlaw(log10amp, gamma) 

        #return the IRN and added GWB for each pulsar, also the VL psd to add to the cross terms later
        return phi_diag.at[self.GWB_fidxs].add(psd_common_total), vl_psd 

    @partial(jax.jit, static_argnums=(0,))
    def get_phi_mat(self, xs):
        """
        Constructs the phi-matrix based on the flattened array of model parameters (`xs`)

        :param xs: flattened array of model paraemters (`xs`)

        :return: the phi-matrix` with dimensions `(Number of Frequencies,Number of pulsars, Number of pulsars)`.
        """
        log10amp, gamma, pulsar_dists, gwb_psd_params = (
            xs[1 : self.num_IR_params : 3],
            xs[0 : self.num_IR_params : 3],
        ...
        phi_diag, vl_psd = self.get_phi_diag(log10amp, gamma, pulsar_dists, gwb_psd_params)
        phi = jnp.zeros((self.int_bins, self.Npulsars, self.Npulsars))
        phi = phi.at[:, self.diag_idx, self.diag_idx].set(phi_diag)

        # Don't need multi-dimensional only 1 orf so it is 2D
        # and the slices are 2D
        # Also, note that: [n_pairs,] * [n_f, 1] = [n_f, n_pairs] which should work to broadcast for frequencies and paired indices 
        return phi.at[self.KGW, self.I, self.J].set((self.orf_val * vl_psd)) #Takes correlations times power at GWB frequencies
        #return  self.orf_val, vl_psd, phi

    # Below is just left alone. Copied from TTVL and left it as is
    # Will need to check to make sure it works 
    @partial(jax.jit, static_argnums=(0,))
    def get_phi_mat_CURN(self, xs):
        """
        Constructs the phi-matrix based on the flattened array of model paraemters (`xs`)
