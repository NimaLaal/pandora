...
class SimGWBFromPhi(object):
    ...
    def __init__(self, phi, freqs, toas, white_sigma, seed = None, psrs = None):
        ...
        if seed:
            random.seed(seed)
        self.rngkey = jar.split(jar.key(random.randint(10, 100000)), num = self.Npulsars + 1)
    ...
    def Fmat(self, pidx):
        ...
        F = F.at[:, 0::2].set(jnp.sin(2 * jnp.pi * self.toas[pidx][:, None] * self.freqs[None, :]))
        return F.at[:, 1::2].set(jnp.cos(2 * jnp.pi * self.toas[pidx][:, None] * self.freqs[None, :]))
    ...
    def get_coeff(self):
        ...
        nu = jar.normal(key = self.rngkey[-1], shape=(self.real, 2*self.nf, self.Npulsars, 1))
    ...
    def sim(self):
        ...
        for pidx in tqdm(range(self.Npulsars)):
            ...
            res.append((R @ (F @ a[..., pidx]))[..., 0] + w * self.white_sigma[pidx][None])
        return res
